...
export class Heatmap {
  ...
  public constructor(
    ...
    d3.select(options.parent).append(
      (): SVGSVGElement =>
        Optional.of(svgNode.node()).orElse(
  ...
  public drawGrid(
    ...
    const colourInterpolator: (t: number) => string = d3.interpolateLab(
      d3.lab(options.colorScale[0]),
      d3.lab(options.colorScale[1]),
    );
    for (const [col, coli] of this.hm
      .columns()
      .map((c: string, i: number) => [c, i])) {
      svg
        .selectAll("svg")
        .data(this.hm.column(col as string).asArray())
        .enter()
        .append("rect")
        .attr(
          "x",
          () =>
            (coli as number) * this.cellShape[0] +
            (coli as number) * options.padding,
        )
        .attr(
          "y",
          (_: Node, i: number) => i * this.cellShape[1] + i * options.padding,
        )
  ...
  public drawLabels(
    ...
    const textStart: [number, number] = [
      this.cellShape[0] * hmShape[1] +
        options.padding * (hmShape[1] - 1) +
        options.textPadding,
      this.cellShape[1] * hmShape[0] +
        options.padding * (hmShape[0] - 1) +
        options.textPadding,
    ];
  ...
  // [width, height]
  public static cellShape(
    options: HeatmapSettings,
    hm: [number, number],
  ): [number, number] {
    const width: number =
      options.width - options.textWidth - hm[1] * options.padding;
    const height: number =
      options.height - options.textHeight - hm[0] * options.padding;
  ...
  public cluster(what: "all" | "columns" | "rows" | "none"): void {
    ...
    if (what == "rows") {
      const cluster: Node = this.clusterFn(
        distanceMatrix(raw.transpose(), this.metricFn),
      );
      cluster.dendsort();
    }
  }
