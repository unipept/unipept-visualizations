 ...
 */
const transpose: <T>(matrix: T[][]) => T[][] = <T>(matrix: T[][]): T[][] => {
  if (matrix.length > 0) {
 ...
 */
const combinations: <T>(data: T[], r: number) => T[][] = <T>(
  data: T[],
  r: number,
): T[][] => {
...
) => Generator<number, void, void> = function* (
  start: number,
  end: number,
...
const copy: <T>(matrix: T[][]) => T[][] = <T>(matrix: T[][]): T[][] =>
  matrix.map((inner: T[]) => inner.slice());

const rad2deg: (rad: number) => number = (rad: number): number =>
  rad * (180 / Math.PI);
