...
const colorFromSettings: (
  ...
  const nodeSize: (data: Node) => number = (data: Node): number =>
    Data.count(data, lens);

  const palette: ScaleOrdinal<string, string> = settings.colors();

  const color: (node: HN) => string = (node: HN): string => {
    ...
    if (node.children !== undefined) {
      ...
      children.sort(
        (a: HN, b: HN): number => nodeSize(b.data) - nodeSize(a.data),
      );
      const childColors: Array<Optional<RGBColor>> = children
        .slice(0, 2) // Only care about the largest 2 children
        .map((child: HN) => Optional.of(rgb(color(child))));

      return averageColor(childColors)
        .map((c: RGBColor | HSLColor) => c.darker(DARKEN_FACTOR).toString())
        .orElse("black");
    }
...
export class Sunburst {
  ...
  public constructor(
    ...
    const dataAccessor: R.Lens = R.lens(
      options.dataAccessor,
      options.dataModifier,
    );

    const svgNode: Selection<
      SVGSVGElement,
      undefined,
      null,
      undefined
    ...
    select(options.parent).append(
      (): SVGSVGElement =>
        Optional.of(svgNode.node())
          .orElse(document.createElement("svg") as unknown as SVGSVGElement));

    const nodeData: HRN[] = Sunburst.initData(
      SunburstNode.createNodes(data),
      dataAccessor,
    );
    ...
    const angularScale: ScaleLinear<number, number> = scaleLinear().range([
      0,
      Math.PI * 2,
    ]); // Use full circle
    ...
    const pathNodes: Selection<
      BaseType,
      HRN,
      SVGGElement,
      undefined
    > = svgGroup.selectAll("path").data(nodeData);
    const textNodes: Selection<
      BaseType,
      HRN,
      SVGGElement,
      undefined
    > = svgGroup.selectAll("text").data(nodeData);
    ...
    this.tooltip = options.enableTooltips
      ? Optional.of(
          new Tooltip(options.parent, options.className, options.getTooltip),
        )
      : Optional.empty();
  ...
  private static initData(data: SunburstNode, accessor: R.Lens): HRN[] {
    ...
    return dataPartition(rootNode).descendants();
  }
  ...
  private onClick(
    ...
    this.animate(datum as HRN, nodes, angularScale, radialScale);
  }
   ...
   */
  private draw(
    pathNodes: Selection<BaseType, HRN, SVGGElement, undefined>,
  ...
  private drawPaths(
    ...
    const opacity: Styles.IStyleConstraints = Styles.constraints({
      fadedOpacity: Sunburst.FADED_NODE_OPACITY,
  ...
  private drawLabels(
    ...
    angularScale: ScaleLinear<number, number>,
    radialScale: ScaleLinear<number, number>): void {
    const style: Styles.IStyleConstraints = Styles.constraints({
      maxDepth,
  ...
  private animatePaths(
    ...
    const angle: (t: number) => number[] = interpolate(angularScale.domain(), [
      parentNode.x0,
      parentNode.x1,
    ]);
    const radius: (t: number) => number[] = interpolate(radialScale.domain(), [
      parentNode.y0,
          Data.outerRadialDomain(parentNode, this.LEVELS)]);
    const yr: (t: number) => number[] = interpolate(radialScale.range(), [
    ...
    paths
      ...
      .attr("fill-opacity", (d: HRN) =>
        Styles.node.opacity(d, maxDepth, style),
      );
  }
    ...
    texts
      ...
      .style("visibility", (child: HRN): string =>
        Styles.displayableFromAncestor(parentNode, child, maxDepth)
          ? "visible" : "hidden")
      .attrTween("text-anchor", (d: HRN) => (): string =>
        Styles.label.anchor(angularScale, d),
      )
      .attrTween("dx", (child: HRN) => (): string =>
        Styles.label.offset(angularScale, child, style),
      )
      .attrTween("transform", (child: HRN) => (): string =>
      ...
      .on("end", function (child: HRN): void {
        select(this)
          ...
          .style(
            "font-size",
            Styles.label.fontSize(child, angularScale, radialScale, style),
          );
      });
  }
}
