...
export class SunburstSettings extends Settings {
  ...
  public readonly countAccessor: (data: Node) => number = this.dataAccessor;

  public readonly rerootCallback?: (data: Node) => void = undefined;

  public readonly getLevel: (data: Node) => number = (data: Node): number =>
    data.getDepth();

  public readonly getLabel: (data: Node) => string = (data: Node): string =>
  ...
  public readonly getTooltipText: (data: Node) => string = (
    data: Node,
  ): string => `${Data.count(data, this.countAccessor)} hits`;

  public readonly getTooltip: (data: Node) => string = (data: Node): string =>
    `<h3 class='tip-title'>${this.getTooltipTitle(data)}</h3>` +
    `<p>${this.getTooltipText(data)}</p>`;

  public readonly getTitleText: (data: Node) => string = (data: Node): string =>
    this.getLabel(data);
