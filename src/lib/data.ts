...
const count: (datum: Node, counter: (d: Node) => number) => number = (
  datum: Node,
  counter: (d: Node) => number,
): number => {
  if (datum.children !== undefined) {
    return datum.children
      .map((v: Node): number => count(v, counter))
...
const countRatio: (
  ...
  denominator: Node,
  counter: R.Lens,
) => number = (numerator: Node, denominator: Node, counter: R.Lens): number =>
...
const maxRadius: (datum: HierarchyRectangularNode<Node>) => number = (
  datum: HierarchyRectangularNode<Node>,
): number =>
 ...
 */
const outerRadialDomain: (
  datum: HierarchyRectangularNode<Node>,
  levels: number,
) => number = (datum: HierarchyRectangularNode<Node>, levels: number): number =>
  Math.min(maxRadius(datum), datum.y0 + levels * (datum.y1 - datum.y0));
...
const ancestorOf: (
  ...
  if (
    child.depth > check.depth &&
    check.children !== undefined &&
    child.parent !== null
  ) {
    return ancestorOf(check, child.parent).map((d: number) => d + 1);
  }
...
class DataFrame<T> {
  ...
  public column(label: string): Series<T> {
    ...
    const size = Math.max(
      0,
      ...this.index.map((label: string) => this.data[label].shape()),
    );
  ...
  public row(label: string): Series<T> {
    return new Series(
      this.index.map((i: string) => this.data[i].at(label)),
      this.columns(),
    );
  }
  ...
  public rows(): string[] {
    const distinct: (
      ...
      index: number,
      self: string[],
    ) => boolean = (value: string, index: number, self: string[]): boolean =>
      self.indexOf(value) === index;

    const allColumns: string[] = this.index.reduce(
      (acc: string[], i: string) => acc.concat(this.data[i].index),
      []);
  ...
  public format(): string {
    const toFormat: DataFrame<string> = this.map((v: T): string =>
      JSON.stringify(v),
    );
    const rowNames: string[] = this.rows();
    const colWidth: number[] = this.index.map((col: string): number =>
      Math.max(
        ...[col.length].concat(
    ...
    const rowWidth: number = Math.max(
      ...rowNames.map((i: string): number => i.length + 1),
    );

    const rowName: (name: string) => string = (name: string): string =>
      `${name.padEnd(rowWidth, " ")} `;

    const colHeadings: string[] = this.index.map((col: string, i: number) =>
      col.padStart(colWidth[i], " "),
    );
    const header
  ...
  public map<U>(f: (value: T) => U): DataFrame<U> {
    return new DataFrame(
      this.index.map((col: string): Series<U> => this.data[col].map(f)),
      this.index);
  }

  public max(lens: R.Lens): R.Ord {
    const maxSeries = R.map(
      (col: string): R.Ord => this.data[col].max(lens),
      this.index,
    );
  ...
  public min(lens: R.Lens): R.Ord {
    const minSeries = R.map(
      (col: string): R.Ord => this.data[col].min(lens),
      this.index,
    );
  ...
  public idxmax(lens: R.Lens): [string, string, T] {
    ...
    const reducer = R.maxBy(
      R.view(valueLens) as (a: [string, string, T]) => R.Ord,
    );
    const data: Array<[string, string, T]> = R.map(
      (col: string) =>
        R.prepend(col, this.data[col].idxmax(lens)) as [string, string, T],
        this.index);
  ...
  public idxmin(lens: R.Lens): [string, string, T] {
    const valueLens = R.compose(R.lensIndex(2), lens) as R.Lens;
    const reducer = R.minBy(
      R.view(valueLens) as (a: [string, string, T]) => R.Ord,
    );
    const data: Array<[string, string, T]> = R.map(
      (col: string) =>
        R.prepend(col, this.data[col].idxmin(lens)) as [string, string, T],
        this.index);
    return R.reduce(reducer, data[0], data) as [string, string, T];
  }
  ...
  public normalise(lens: R.Lens): DataFrame<T> {
    ...
    if (typeof max === "number") {
      return this.map(
        (node: T): T => R.over(lens, (val: number): number => val / max, node),
      );
    } else {
      return this;
    }
  ...
  public reorderColumns(newIndex: string[]): DataFrame<T> {
    ...
    return new DataFrame(
      constrainedIndex.map((col: string): Series<T> => this.data[col]),
      constrainedIndex);
  }
  ...
  public reorderRows(newIndex: string[]): DataFrame<T> {
    return new DataFrame(
      this.index.map(
        (col: string): Series<T> => this.data[col].reorder(newIndex),
      ),
      this.index,
    );
  }
...
const fromCSV: (data: CSV) => DataFrame<Node> = (
  ...
  const seriesIndex: string[] = data.map(
    (row: CSVRow): string => row[seriesIndexName],
  );

  return new DataFrame<Node>(
    data.columns.slice(1).map(
      (col: string): Series<Node> =>
        new Series(
          data.map(
            (row: CSVRow, i: number): Node => {
...
const fromArray: <T>(data: T[][]) => DataFrame<T> = <T>(
  data: T[][],
): DataFrame<T> =>
